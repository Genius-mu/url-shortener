...
// import toast from "react-hot-toast";
// import { PopUpFunction } from "./UrlShortener";

// export default function UrlCard({ url, base, onCopy, onDelete }) {
//   const full = `${base}/${url.short_code}`;
//   const { BtnPop, setBtnPop } = useContext(PopUpFunction);
...
//       <div className="flex-1">
//         <p className="text-sm text-gray-800 break-all text-[12px] sm:text-[15px] sm:max-w-[350px]">
//           {url.original_url}
//         </p>
//         <a
...
export default function UrlCard({ url, base, onCopy, onDelete }) {
  ...
  const { setBtnPop, setSelectedURL } = useContext(PopUpFunction);

  return (
    <div className="bg-white/40 backdrop-blur-xl p-4 rounded-2xl border border-white/40 shadow-md flex items-start justify-between flex-col sm:flex-row gap-y-3">
      <div className="flex-1">
        <p className="text-sm text-gray-800 break-all text-[13px] sm:max-w-[350px]">
          {url.original_url}
        </p>
        <a
          className="text-blue-700 font-semibold block mt-2 text-[13px]"
          href={full}
          target="_blank"
          rel="noreferrer"
        >
        ...
        <p className="text-xs text-gray-600 mt-1">
          Clicks: {url.clicks} • {new Date(url.created).toLocaleString()}
        </p>
      </div>
